       ...
       if (! d->wave_edit->waveform->selected)
           {
            if (d->wave_edit->waveform->sound_buffer->buffer_offset >= 
               d->wave_edit->waveform->sound_buffer->samples_total - nsamples) 
              {
               ...
               d->wave_edit->waveform->cursor_position = 0;
              }                                          
            }
       else //looped, but selected
           {
            if (d->wave_edit->waveform->sound_buffer->buffer_offset >= 
               ...
               { 
                ...
                d->wave_edit->waveform->set_cursor_to_sample (d->wave_edit->waveform->sample_start());
               }
           }
      }
...
void CEKO::create_paths()
{
  ...
  fname_tempfile = "/eko-temp-777.wav";   //append (QDir::tempPath()).append ("/eko-temp-777.wav");
...
{
 // resampling_realtime_quality = cmb_src->currentIndex();
  //settings->setValue ("resampling_realtime_quality", resampling_realtime_quality);
...
void CEKO::open()
{
 // qDebug() << "CEKO::open() - start";
  ...
  dialog.setNameFilter (tr ("All (*);;WAV files (*.wav);;Compressed files (*.ogg *.flac)"));
...
void CEKO::createMenus()
{
 ...
 // add_to_menu (tm, tr ("Export to RAW"), SLOT(file_export_raw()));
  ...
  tm = menu_functions->addMenu (tr ("Amplification"));
  tm->setTearOffEnabled (true);
...
/*
void CEKO::cmb_sound_api_currentIndexChanged (int index)
{
  cmb_sound_dev_out->clear(); 
  cmb_sound_dev_out->addItems (get_devices_for_api (index));
  cmb_sound_dev_in->clear(); 
  cmb_sound_dev_in->addItems (get_input_devices_for_api (index));

  settings->setValue ("sound_api", index);
...
/*
  QHBoxLayout *lt_src = new QHBoxLayout;

  QLabel *l_src = new QLabel (tr ("Resampling quality for playback"));
  cmb_src = new QComboBox;

  QStringList sl_src_modes;

  sl_src_modes.append (tr ("Best"));
  sl_src_modes.append (tr ("Medium"));
  sl_src_modes.append (tr ("Fastest"));
  sl_src_modes.append (tr ("Zero Order Hold"));
  sl_src_modes.append (tr ("Linear"));

  cmb_src->addItems (sl_src_modes);

  cmb_src->setCurrentIndex (settings->value ("resampling_realtime_quality", SRC_LINEAR).toInt());


  lt_src->addWidget (l_src);
  lt_src->addWidget (cmb_src);
*/
 ...
 // page_common_layout->addLayout (lt_src);
...
/*
  QHBoxLayout *hb_soundev_api = new QHBoxLayout;

  QLabel *l_soundev_api = new QLabel (tr ("API"));

  
  cmb_sound_api = new QComboBox;
  cmb_sound_api->addItems (lpa_apis);

  
  hb_soundev_api->addWidget (l_soundev_api);
  hb_soundev_api->addWidget (cmb_sound_api);

  pa_api = settings->value ("sound_api",  Pa_GetDefaultHostApi()).toInt();

  cmb_sound_api->setCurrentIndex (pa_api);
  page_soundev_layout->addLayout (hb_soundev_api);
*/
...
void CEKO::nav_save_pos()
{
}


void CEKO::nav_goto_pos()
{
}
...
//int pa_device_id;
      ...
      PaError err =  Pa_OpenStream 	(&pa_stream,
       ...
       else //just set the offset to the cursor position
            d->wave_edit->waveform->sound_buffer->buffer_offset = d->wave_edit->waveform->cursor_position *
                                                                  d->wave_edit->waveform->frames_per_section *
                                                                  d->wave_edit->waveform->sound_buffer->channels;
...
//idea has been borrowed from Muse
  ...
  for (size_t i = start; i < end; i++)
      {
       if (max_peak < d->wave_edit->waveform->sound_buffer->buffer[i])
...
void CEKO::fn_insert_silence()
{
  ...
  d->wave_edit->waveform->sound_buffer->paste_at (buf, 
                                                  d->wave_edit->waveform->cursor_pos_sample());
...
void CEKO::fn_detect_average_value()
{
  ...
  log->log (tr ("max level in samples = %1").arg (QString::number (maxpeak)));
  log->log (tr ("max level in dB = %1").arg (QString::number (float2db (maxpeak))));
  log->log (tr ("min level in samples = %1").arg (QString::number (minpeak)));
...
void CEKO::file_record()
{
 ...
 // int format = settings->value ("def_sndfile_format", sndfile_format).toInt();
  ...
  //file_temp_handle = sf_open (fname_tempfile.toLatin1(), SFM_WRITE, &sf);
...
*/
      ...
      inputParameters.device = pa_device_id_in;
      inputParameters.channelCount = 2;//channels;
      inputParameters.sampleFormat = paFloat32;
      inputParameters.suggestedLatency = Pa_GetDeviceInfo (inputParameters.device)->defaultLowOutputLatency;;
      inputParameters.hostApiSpecificStreamInfo = NULL;
...
qDebug() << "simple_counter: " << simple_counter;
...
/*
  ...
  bool ok;
  int percents = QInputDialog::getInt (this, tr ("?"),
                                             tr("Stretch to per-cents"), 50, 1, 1000, 1, &ok);
