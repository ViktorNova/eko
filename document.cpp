...
//  size_t size_in_samples = sample_end - sample_start;
...
//  if (seconds > 60)
  ...
  for (size_t x = 0; x < sections; x++)
      ...
      if (measure_in_seconds)
         ...
         //if (! (x % (int)ceil (sections_per_second * 60)))
         // if (x % 100 == 0)
         if (x % 60 == 0)
            {
             QPoint p1 (x, 1);
...
/*
  qDebug() << "sections total: " << sections_total;
  qDebug() << "from: " << section_from << " to: " << section_to;
  qDebug() << "sections to draw: " << sections;
  qDebug() << "frames_per_section: " << frames_per_section;

  qDebug() << "channels = " << channels;
  qDebug() << "channel_height = " << channel_height;
  */
  ...
  for (size_t x = 0; x < sections; x++)
      {
       int ymax = channel_height - (int)((minmaxes->values[ch]->max_values->samples[x] + 1) * .5 * channel_height);
       int ymin = channel_height - (int)((minmaxes->values[ch]->min_values->samples[x] + 1) * .5 * channel_height);
...
//draw the amplitude meter bar  
  ...
  int ten = get_value (channel_height / 2, 10);  
            ...
            if (! (y % ten))
               {
                ...
                ff = (1.0f / 100) * percentage/* + 0.1*/;
                                            
                QPoint p1 (1, y + channel_height * ch);
                QPoint p2 (5, y + channel_height * ch);
                painter.drawLine (p1, p2);
                ...
                if (draw_shadow)
                   {    
                    ...
                    if (! show_db)
                        painter.drawText (QPoint (2, 1 + y + channel_height * ch), QString::number (ff));  //in normalized float format
                    else    
                        painter.drawText (QPoint (2, 1 + y + channel_height * ch), QString::number (float2db (ff), 'g', 2)); //dB
  
                    painter.setPen (cl_text);
                   }                  
                   
                if (! show_db)
                    painter.drawText (QPoint (1, y + channel_height * ch), QString::number (ff));  //in normalized float format
                else    
                    painter.drawText (QPoint (1, y + channel_height * ch), QString::number (float2db (ff), 'g', 2)); //dB
...
void CWaveform::paintEvent (QPaintEvent *event)
{
  ...
  if (selection_start != selection_end)
     {
      int s_start = selection_start - section_from;  //section start
      int s_end = selection_end - section_from;  //section end
...
{
  int pos_in_samples = sound_buffer->buffer_offset;
  int pos_in_frames = pos_in_samples / sound_buffer->channels;
  
  set_cursor_value (pos_in_frames / frames_per_section);
   
  if (cursor_position > section_to) 
     {
...
void CWaveform::update_cursor_pos()
{
  int pos_in_samples = sound_buffer->buffer_offset;
  int pos_in_frames = pos_in_samples / sound_buffer->channels;
...
CWaveform::CWaveform (QWidget *parent): QWidget (parent)
{
  ...
  timer.setInterval (40); //сделать зависимым от текущего масштаба!
...
{ 
  ...
  sel_start_samples = cursor_samples;
  sel_end_samples = cursor_samples;
}
...
void CWaveform::mousePressEvent (QMouseEvent *event)  
{
  ...
  int section = section_from + x;

//deselect if click is not on the selection bounds
  if (! x_nearby (section, selection_start, 2) &&
      ! x_nearby (section, selection_end, 2))
     {
  ...
  else
      {
       ...
       if (x_nearby (section, selection_end, 2))
...
void CWaveform::mouseMoveEvent (QMouseEvent *event)
{
  ...
  if (! mouse_pressed)
  {
  if (x_nearby (current_section, selection_start, 2) ||
      x_nearby (current_section, selection_end, 2))
     {
...
*/
  ...
  if (frames_per_section != 0)
     {
    //  cursor_position = cursor_frames / frames_per_section;
      cursor_position = cursor_samples / sound_buffer->channels / frames_per_section;
      selection_start = sel_start_samples / sound_buffer->channels / frames_per_section;
      selection_end = sel_end_samples / sound_buffer->channels / frames_per_section;
     }
...
//      documents->transport_control->call_play_pause();
  ...
  if (event->key() == Qt::Key_Return)
     {
      ...
      //documents->transport_control->call_stop();
      event->accept();   
      return;
     }
  ...
  if (event->key() == Qt::Key_End)
     {
      ...
      if (event->modifiers() & Qt::ShiftModifier)  
         {
          selection_end = sections_total - 1;
          selected = true;   
         }
  ...
  if (event->key() == Qt::Key_Left)
     {
      ...
      if (cursor_position == section_from)
         {
      ...
      if (event->modifiers() & Qt::ShiftModifier)  
         {
          if (selection_start != 0)
             set_selstart_value (selection_start - 1);
  ...
  if (event->key() == Qt::Key_Right)
     {
      ...
      if (cursor_position == sections_total)
          cursor_position--;
      ...
      if (cursor_position == section_to)
       ...
       if (event->modifiers() & Qt::ShiftModifier)  
          {
           if (selection_end != sections_total)
              selection_end++;
  ...
  if (event->text() == "[")
      {
       set_selstart_value (cursor_position);
       fix_selection_bounds();
  ...
  if (event->text() == "]")
      {
       set_selend_value (cursor_position);
...
void CWaveform::paste()
{
  ...
  if (cursor_position == 0)
     set_cursor_value (cursor_position + 1);

  sound_buffer->paste_at (sound_clipboard, cursor_pos_sample());
  deselect();
...
void CWaveform::fix_selection_bounds()
{
  if (selection_start > selection_end)
     {
      size_t t_sections = selection_end;
      size_t t_samples = sel_end_samples;

      selection_end = selection_start;
      sel_end_samples = sel_start_samples;

      selection_start = t_sections;
      sel_start_samples = t_samples;
   }
...
void CWaveform::undo_take_shot (int type, int param)
{
  ...
  el->sample_cursor = cursor_samples;
  ...
  if (type == UNDO_PASTE)
     {
      el->sample_start = cursor_pos_sample();
...
{
  if (frames_per_section == 0 ||
      sound_buffer->channels == 0
     )
    return;

  cursor_position = sampleno / frames_per_section / sound_buffer->channels;
  cursor_samples = sampleno;
}


size_t CWaveform::cursor_pos_sample()
{
  return cursor_position * frames_per_section * sound_buffer->channels;
}
...
void CWaveform::set_cursorpos_text()
{
 // qDebug() << "CWaveform::set_cursorpos_text()";
  ...
  size_t frames = cursor_position * frames_per_section;
...
void CDocument::create_new()
{
  ...
  //int tab_index = documents->tab_widget->addTab (wave_edit, file_name);


  tab_page = holder->tab_widget->widget (tab_index);
  //tab_page = documents->tab_widget->widget (tab_index);
...
//  return documents->tab_widget->indexOf (tab_page);
...
CFxRackWindow::CFxRackWindow()
{
  ...
  h_mainbox->addWidget (level_meter); 
...
//get max values
  ...
  if (documents->current->wave_edit->waveform->sound_buffer->channels == 2)
     do
       {
        if (maxl < temp_buffer[i])
            maxl = temp_buffer[i];
        i++;
        if (maxr < temp_buffer[i])
            maxr = temp_buffer[i];
        i++;
       }
    while (i < nsamples);
